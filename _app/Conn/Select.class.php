...
class Select extends Conn {
    ...
    /**
     * <b>ExeSelect:</b> Executa uma leitura simplificada no banco de dados utilizando prepared statements.
     * Basta informar o nome da tabela os termos da seleção e uma analize em cadeia (ParseString) para executar. 
     * 
     * @param STRING $Tabela Informe o nome da tabela no banco!
     * @param STRING $Campo = campo1, campo2, campo3...
     * @param STRING $Termos = WHERE | ORDER | LIMIT :limit | OFFSET :offset
     * @param STRING $ParseString = link={$link}&link2={$Link2}
     */
    public function ExeSelect($Tabela, $Campos = null, $Termos = null, $ParseString = null) {

        $this->Campos = (!empty($Campos) ? $Campos : "*");

        if (!empty($ParseString)) {
            parse_str($ParseString, $this->Places);
        }
        // Prepara a select
        $this->Select = "SELECT {$this->Campos} FROM {$Tabela} {$Termos}";
        //Executa a query
        $this->Execute();
    ...
    /**
     * <b>Full Select</b> Executa leitura de dados via query que deve ser montada manualmente para possibilitar
